...
#if C_Debug
#define Debug_print(fmt, args...)  { printf("[CCA SPI][%s][%06d]    ",__FUNCTION__, __LINE__); printf( fmt, ## args); }
#else
...
//#define CA_SYSTEM_CALL_DEBUG 1

#if CA_SYSTEM_CALL_DEBUG
#define Debug_System_print(fmt, args...)  { printf("[CCA SPI][%s][%06d]    ",__FUNCTION__, __LINE__); printf( fmt, ## args); }
#else
#define Debug_System_print(fmt, args...)  
#endif
...
#if (WDT_ENABLE == TRUE)
static uc_bool WDT_CLEAR_FLAG = TRUE;

inline  void setWDTResetFlag(uc_bool f_bool)
{
    WDT_CLEAR_FLAG = f_bool;
}
inline uc_bool getWDTResetFlag(void)
{
    return WDT_CLEAR_FLAG;
}

static void WDT_Cb(uc_uint32 u32Vector)
{
    if(getWDTResetFlag())
    {
        MDrv_WDT_Clear();
...
{
    WDT_Result Result;
    MsOS_AttachInterrupt(E_INT_FIQ_WDT, (InterruptCb)WDT_Cb);
    MsOS_EnableInterrupt(E_INT_FIQ_WDT);
    MDrv_WDT_ClearRstFlag();
    Result = MDrv_WDT_IsReset();
    MDrv_WDT_SetTimer(E_WDT_DBGLV_ALL, 40);
...
void MApi_WDT_Stop(void)
{
    MDrv_WDT_ClearRstFlag();
    MDrv_WDT_Clear();
    MDrv_WDT_SetTimer(E_WDT_DBGLV_ALL, 0xFFFF);
    MDrv_WDT_Stop(E_WDT_DBGLV_INFO);
}
#endif


static void _SMC_Notify(uc_uint8 u8SCID, SC_Event eEvent)
{
    ...
    switch (eEvent)
    {
        case E_SC_EVENT_IN:
            ...
            if(Sc_Info.pfnNotify != 0)
            {
                UniversalClientSPI_Smartcard_Reset(u8SCID);
            }
            break;
        case E_SC_EVENT_OUT:
            ...
            if(Sc_Info.pfnNotify != 0)
            {
                Sc_Info.pfnNotify(u8SCID, Sc_Info.smartcardHandle, UC_SC_CARD_OUT, NULL);
            }
            break;
...
static uc_bool _MS_Start_KeyLadder(uc_uint32 ecmDescHandle, DSCMB_Key_Type ecmKeyType, const uc_key_info *pKeyInfo)
{
	DSCMB_KL_Status KL_Status = 0;
	uc_bool ret = FALSE;
	static uc_uint8 u8EncryptedCW[48];
	static uc_uint8 ACPU_Out[16] = {0x00};
	memset(u8EncryptedCW,0,48);
	memcpy(u8EncryptedCW,&s_spi_ucSystem.strCSSK[0], CSSK_DATA_SIZE);
	if(pKeyInfo->pDescramblingKey->length == 8)
	{
		memcpy(u8EncryptedCW+16,pKeyInfo->pDescramblingKey->bytes,pKeyInfo->pDescramblingKey->length);
		memcpy(u8EncryptedCW+24,pKeyInfo->pDescramblingKey->bytes,pKeyInfo->pDescramblingKey->length);
	}
	else
	{
		memcpy(u8EncryptedCW+16,pKeyInfo->pDescramblingKey->bytes,pKeyInfo->pDescramblingKey->length);
	...
	printf("[%s][%d]YJC key info\n",__func__,__LINE__);
	uc_uint8 c = 0;
	printf("sessino key start\n");
	for(c=0;c<CSSK_DATA_SIZE;c++)
	{
		printf("0x%x ",s_spi_ucSystem.strCSSK[c]);
	}
	printf("\nsession key end\n");
	...
	DSCMB_KLCfg_All KLConfigAll =
	{
		...
		.eSrc = E_DSCMB_KL_SRC_SECRET_1,
		.eDst = E_DSCMB_KL_DST_KT_ESA,	//Set to Key Table ESA for DSCMB
		.eOutsize = E_DSCMB_KL_128_BITS,
		.eKeyType = ecmKeyType,   //even or odd
		.u32Level = 0x2,					//Set to 2-level
		.u32EngID = 0x0,		//Set to zero in STB chip
		.u32DscID = ecmDescHandle,
		.u8KeyACPU = NULL,
    ...
    if(pKeyInfo->descramblingKeyProtection == UC_DK_PROTECTION_AES)
    {
...
void _MS_EMM_SecCallBackFn(uc_uint8 u8DmxId, DMX_EVENT enEvent)
{
    int i = 0;


//    printf("u8DmxId 0x%x enEvent 0x%x\n",u8DmxId,enEvent);

    for(i = 0; i < MAX_EMM_FLT_NUM; i++)
    {
        if(gEMMStreamInfo.filterHandle[i] == u8DmxId)
        {
            break;
        }
    }
    ...
    else
    {
        ...
        {
            uc_uint32 u32EventFlag = E_DMX_EMM_CB;
            u32EventFlag |= EMM_FLT_MASK(i);
            MsOS_SetEvent(_s32CBMonitorEMMEventId, u32EventFlag);
        }
...
void _MS_ECM_SecCallBackFn(uc_uint8 u8DmxId, DMX_EVENT enEvent)
{
    int i = 0;

    for(i = 0; i < MAX_ECM_FLT_NUM; i++)
    {
        if(gECMStreamInfo.filterHandle[i] == u8DmxId)
        {
            break;
        }
    }

    if(i >= MAX_ECM_FLT_NUM)
    {
        return;
    }
    else
    {
        if(enEvent == DMX_EVENT_DATA_READY)
        {
            uc_uint32 u32EventFlag = E_DMX_ECM_CB;
            u32EventFlag |= ECM_FLT_MASK(i);
            MsOS_SetEvent(_s32CBMonitorEventId, u32EventFlag);
        }
        else if(enEvent == DMX_EVENT_BUF_OVERFLOW)
        {
            printf("---> ECM Overflow\n");
...
void _MS_CB_EMMMonitorTask(void)
{
    uc_uint32 u32Events;
    int i = 0;
    ...
    while(1)
    {
        MsOS_WaitEvent(_s32CBMonitorEMMEventId, E_DMX_CB, &u32Events, E_OR, MSOS_WAIT_FOREVER);

        //EMM
        if(u32Events & E_DMX_EMM_CB)
        {
            for(i = 0; i < MAX_EMM_FLT_NUM; i++)
            {
                if(u32Events & EMM_FLT_MASK(i))
                {
 //                   DMX_FILTER_STATUS retStatus = DMX_FILTER_STATUS_ERROR;
                    uc_uint8 *pDataBuf = gEMMStreamInfo.filterBuffer[i];
                    uc_uint8 u8DmxId = gEMMStreamInfo.filterHandle[i];
                    uc_uint32 u32DataLen = 0;
                    uc_uint32 u32RmnSize = 0;
                    uc_uint32 streamIdx = 0xFFFFFFFF;

                    streamIdx = gEMMStreamInfo.filterToStream[i];


                    
                    
                    MS_PHYADDR readpoint,writepoint;
                    uc_uint32 bufSize = 0;
                    uc_uint32 totalLen = 0;
                    
                    MApi_DMX_SectReadGet(u8DmxId,&readpoint);
                    MApi_DMX_SectWriteGet(u8DmxId,&writepoint);
                    
                    if(writepoint<readpoint)
                    {
                         uc_uint32 startpoint,endpoint;
                    
                         MApi_DMX_SectStartGet(u8DmxId,&startpoint);
                         MApi_DMX_SectEndGet(u8DmxId,&endpoint);
                    
                         bufSize = (endpoint - readpoint) + (writepoint - startpoint);
                    }
                    else
                    {
                    ...
                    if(bufSize == 0)
                    {
                        printf("[%s][%d] no data need to copy \n",__func__,__LINE__);
                        MsOS_ClearEvent(_s32CBMonitorEMMEventId, EMM_FLT_MASK(i));
                        continue;
                    }

//                    printf("[%s][%d] before dmx databuf 0x%x\n",__func__,__LINE__,bufSize);

                    while((totalLen<bufSize))
                    {                   
                        if(DMX_FILTER_STATUS_OK != MApi_DMX_CopyData(u8DmxId, pDataBuf, EMM_FLT_BUF_SIZE, &u32DataLen, &u32RmnSize, NULL))
                        {
                            printf("[%s][%d] yjc copy data fail\n",__func__,__LINE__);
                            MApi_DMX_SectReset(u8DmxId);
                            break;
                        }

                        gEMMStreamInfo.byteBufferStruct[i].bytes = pDataBuf;
                        gEMMStreamInfo.byteBufferStruct[i].length = u32DataLen;
                        gEMMStreamInfo.notifyCallback[streamIdx](gEMMStreamInfo.connectionHandle[streamIdx], &gEMMStreamInfo.byteBufferStruct[i]);
                        totalLen+= u32DataLen;   
                        if(u32RmnSize == 0)
                        {
                            break;
...
//                    printf("[%s][%d] after dmx totalLen 0x%x\n",__func__,__LINE__,totalLen);
                    MsOS_ClearEvent(_s32CBMonitorEMMEventId, EMM_FLT_MASK(i));
                }
            }
            MsOS_ClearEvent(_s32CBMonitorEMMEventId, E_DMX_EMM_CB);
        }
    }
...
void _MS_CB_MonitorTask(void)
{
    ...
    while(1)
    {
        MsOS_WaitEvent(_s32CBMonitorEventId, E_DMX_CB, &u32Events, E_OR, MSOS_WAIT_FOREVER);

        //ECM
        if(u32Events & E_DMX_ECM_CB)
        {
            for(i = 0; i < MAX_ECM_FLT_NUM; i++)
            {
                if(u32Events & ECM_FLT_MASK(i))
                {
//                    DMX_FILTER_STATUS retStatus = DMX_FILTER_STATUS_ERROR;
                    uc_uint8 *pDataBuf = gECMStreamInfo.filterBuffer[i];
                    uc_uint8 u8DmxId = gECMStreamInfo.filterHandle[i];
                    uc_uint32 u32DataLen = 0;
                    uc_uint32 u32RmnSize = 0;
                    ...
                    {
                        printf("[%s][%d] no data need to copy \n",__func__,__LINE__);
                        MsOS_ClearEvent(_s32CBMonitorEventId, ECM_FLT_MASK(i));
                        continue; 
                    }
...
//                    printf("[%s][%d] before dmx databuf 0x%x\n",__func__,__LINE__,bufSize);
                    ...
                    while((totalLen<bufSize))
                    {
                        ...
                        gECMStreamInfo.byteBufferStruct[i].bytes = pDataBuf;
                        gECMStreamInfo.byteBufferStruct[i].length = u32DataLen;
...
//                    printf("[%s][%d] after dmx totalLen 0x%x\n",__func__,__LINE__,totalLen);
            ...
            MsOS_ClearEvent(_s32CBMonitorEventId, E_DMX_ECM_CB);
        }
    }
        ...
        if(u32Events & E_DMX_EMM_CB)
        {
            for(i = 0; i < MAX_EMM_FLT_NUM; i++)
            {
                if(u32Events & EMM_FLT_MASK(i))
                {
                    ...
                    retStatus = MApi_DMX_CopyData(u8DmxId, pDataBuf, EMM_FLT_BUF_SIZE, &u32DataLen, &u32RmnSize, NULL);
                    if(retStatus != DMX_FILTER_STATUS_OK)
                    {
                        printf("copy data err\n");
                    }
                    else
                    {
                        if(u32DataLen > EMM_FLT_BUF_SIZE || u32DataLen <= 0)
                        {
                            MApi_DMX_SectReset(u8DmxId);
                        }
                        else
                        {
                            gEMMStreamInfo.byteBufferStruct[i].bytes = pDataBuf;
                            gEMMStreamInfo.byteBufferStruct[i].length = u32DataLen;
                            //Debug_print("EMM Task i = %d data len:(%d) buf:(%p)\n", i, gEMMStreamInfo.byteBufferStruct[i].length, gEMMStreamInfo.byteBufferStruct[i].bytes);
                            gEMMStreamInfo.notifyCallback[streamIdx](gEMMStreamInfo.connectionHandle[streamIdx], &gEMMStreamInfo.byteBufferStruct[i]);
                    ...
                    MsOS_ClearEvent(_s32CBMonitorEventId, EMM_FLT_MASK(i));
                }
            }
            MsOS_ClearEvent(_s32CBMonitorEventId, E_DMX_EMM_CB);
        }
        ...
        if(u32Events & E_DMX_ECM_CB)
        {
            for(i = 0; i < MAX_ECM_FLT_NUM; i++)
            {
                if(u32Events & ECM_FLT_MASK(i))
                {
                    ...
                    retStatus = MApi_DMX_CopyData(u8DmxId, pDataBuf, ECM_FLT_BUF_SIZE, &u32DataLen, &u32RmnSize, NULL);
                    if(retStatus != DMX_FILTER_STATUS_OK)
                    {
                        printf("copy data err\n");
                    }
                    ...
                    {
                        if(u32DataLen > ECM_FLT_BUF_SIZE || u32DataLen <= 0)
                        {
                            MApi_DMX_SectReset(u8DmxId);
                        }
                        else
                        {
                            gECMStreamInfo.byteBufferStruct[i].bytes = pDataBuf;
                            gECMStreamInfo.byteBufferStruct[i].length = u32DataLen;
                            //Debug_print("ECM Task i = %d data len:(%d) buf:(%p)\n", i, gECMStreamInfo.byteBufferStruct[i].length, gECMStreamInfo.byteBufferStruct[i].bytes);
                            gECMStreamInfo.notifyCallback[i](gECMStreamInfo.connectionHandle[i], &gECMStreamInfo.byteBufferStruct[i]);
...
uc_bool _MS_Monitor_Task_Init(void)
{
    if(_s32CBMonitorIsStarted != 0)
    {
        return TRUE;
    }
    else
    {
        _s32CBMonitorIsStarted = 1;
    }

    if(_s32CBMonitorEventId < 0)
    {
        _s32CBMonitorEventId = MsOS_CreateEventGroup((char *)"CB_EVENT");
    }

    if(_s32CBMonitorEMMEventId < 0)
    {
        _s32CBMonitorEMMEventId = MsOS_CreateEventGroup((char *)"CB_EMM_EVENT");
    }


    if(_s32CBMonitorEventId < 0)
    {
        UC_SPI_ERR("CB_EVENT create FAIL !\n");
        return FALSE;
    }
    else if(_s32CBMonitorEMMEventId < 0)
    {
        if(MsOS_DeleteEventGroup(_s32CBMonitorEventId) == FALSE)
        {
            UC_SPI_ERR("Delete CB_EVENT failed.\n");
        }
        _s32CBMonitorEventId = -1;
        UC_SPI_ERR("CB_EMM_EVENT create FAIL !\n");
        return FALSE;

    }     
    else
    {
        if(_s32CBMonitorTaskId < 0)
        {
            _s32CBMonitorTaskId = MsOS_CreateTask(
                                      (TaskEntry)_MS_CB_MonitorTask,
                                      (uc_uint32)0,
                                      E_TASK_PRI_HIGHEST,
                                      TRUE,
                                      s_u8CBMonitorStack,
        ...
        if(_s32CBMonitorTaskId < 0)
        {
            UC_SPI_ERR("Create CB MONITOR TASK FAIL \n");
            return FALSE;
        }


        if(_s32CBMonitorEMMTaskId < 0)
        {
            _s32CBMonitorEMMTaskId = MsOS_CreateTask(
                                  (TaskEntry)_MS_CB_EMMMonitorTask,
                                  (uc_uint32)0,
                                  E_TASK_PRI_HIGHEST,
                                  TRUE,
                                  s_u8CBMonitorEMMStack,
                                  4096,
                                  "CB MONITOR TASK"
                              );
        }
        ...
        if(_s32CBMonitorEMMTaskId < 0)
        {
           if(MsOS_DeleteEventGroup(_s32CBMonitorEventId) == FALSE)
            ...
            UC_SPI_ERR("Create CB EMM MONITOR TASK FAIL \n");
            return FALSE;
        }
...
uc_bool _MS_Monitor_Task_DeInit(void)
{
    if(_s32CBMonitorEventId >= 0)
    {
        if(MsOS_DeleteEventGroup(_s32CBMonitorEventId) == FALSE)
    ...
    if(_s32CBMonitorTaskId >= 0)
    {
        if (MsOS_DeleteTask(_s32CBMonitorTaskId) == FALSE)
        {
            UC_SPI_ERR("Delete CB MONITOR TASK failed.\n");
            return FALSE;
        }
        _s32CBMonitorTaskId = -1;
    }


    if(_s32CBMonitorEMMEventId >= 0)
    {
        if(MsOS_DeleteEventGroup(_s32CBMonitorEMMEventId) == FALSE)
        {
            UC_SPI_ERR("Delete CB_EMMEVENT failed.\n");
        }
        _s32CBMonitorEMMEventId = -1;
    }

    if(_s32CBMonitorEMMTaskId >= 0)
    {
        if (MsOS_DeleteTask(_s32CBMonitorEMMTaskId) == FALSE)
        {
            UC_SPI_ERR("Delete CB MONITOR TASK failed.\n");
            return FALSE;
        }
        _s32CBMonitorEMMTaskId = -1;
    }

    
    _s32CBMonitorIsStarted = 0;
...
// Add by junchi.lin 20140624

uc_uint16 u16PlatformIdentifiersParam = 0;
uc_uint16 u16PriPerDataParam = 0;

void Ird_SetSmartCardParamsForT14(SC_Param paramT14)
{
    astScParam[0].u8Protocal   = paramT14.u8Protocal;
    astScParam[0].eCardClk     = paramT14.eCardClk;
    astScParam[0].u8UartMode   = paramT14.u8UartMode;
    astScParam[0].u16ClkDiv    = paramT14.u16ClkDiv;
    astScParam[0].eVccCtrl     = paramT14.eVccCtrl;
    astScParam[0].u16Bonding   = paramT14.u16Bonding;
    astScParam[0].pfOCPControl = paramT14.pfOCPControl;
    astScParam[0].u8Convention = paramT14.u8Convention;

    printf("T14 Param: \n");
    printf("Card Clock: %x \n", astScParam[0].eCardClk);
    printf("Uart Mode:  %x\n", astScParam[0].u8UartMode);
    printf("Clock Div:  %x\n", astScParam[0].u16ClkDiv);
    printf("Vcc Ctrol:  %x\n", astScParam[0].eVccCtrl);
}


void Ird_SetSmartCardParamsForT0(SC_Param paramT0)
{
    astScParam[1].u8Protocal   = paramT0.u8Protocal;
    astScParam[1].eCardClk     = paramT0.eCardClk;
    astScParam[1].u8UartMode   = paramT0.u8UartMode;
    astScParam[1].u16ClkDiv    = paramT0.u16ClkDiv;
    astScParam[1].eVccCtrl     = paramT0.eVccCtrl;
    astScParam[1].u16Bonding   = paramT0.u16Bonding;
    astScParam[1].pfOCPControl = paramT0.pfOCPControl;
    astScParam[1].u8Convention = paramT0.u8Convention;

    printf("T0 Param: \n");
    printf("Card Clock: %x \n", astScParam[1].eCardClk);
    printf("Uart Mode:  %x\n", astScParam[1].u8UartMode);
    printf("Clock Div:  %x\n", astScParam[1].u16ClkDiv);
    printf("Vcc Ctrol:  %x\n", astScParam[1].eVccCtrl);
}

void Ird_DebugParam(uc_uint32 debugParam)
{
    switch(debugParam)
    {
        case 0:
        {
            u16PlatformIdentifiersParam = 0;
            u16PriPerDataParam = 0;
            break;
        }

        case 1:
        {
            u16PlatformIdentifiersParam = 1;
            u16PriPerDataParam = 1;
            break;
        }
...
{
    Debug_print("[START] GetVersion %d \n" ,UNIVERSALCLIENTSPI_VERSION );
...
void* UniversalClientSPI_Memory_Malloc(uc_uint32 bytes)
{
    ...
    else
    {
	    ...
	    if(pvMem == NULL)
	    {
	        Debug_System_print("[END] Allocate Memory fail . NULL point! \n");
	    }
	    Debug_System_print("[%d][%s][END] addr[%p] size[%d]\n", __LINE__, __FUNCTION__, pvMem, bytes);
	    return pvMem;
    }
}
...
void  UniversalClientSPI_FatalError(uc_fatal_error_type type,void* lpVoid)
{
    ...
    {
        Debug_System_print("UC_FATAL_ERROR_PS_DAMAGED::%s\n", (char *)lpVoid);
    }
    else if(type == UC_FATAL_ERROR_MEMORY)
    {
        Debug_System_print("UC_FATAL_ERROR_MEMORY::%s\n", (char *)lpVoid);
    }
    else
    {
        Debug_System_print("UNKONWN Fatal Error Type:%d ::%s\n",type ,(char *)lpVoid);
    }
...
{
    Debug_System_print("[START] initialValue:%u\n" , initialValue );
...
#endif
    //*pSemaphoreHandle = (uc_semaphore_handle)sRest;

    Debug_System_print("[END] Handle:%lu\n",*pSemaphoreHandle );
...
{
    Debug_System_print("[START]  Semaphore_handle:%lu\n" , (uc_sint32)semaphoreHandle );
...
{
    Debug_System_print("[START] Semaphore_handle:%lu  Milliseconds:%lu .\n" , (uc_sint32)semaphoreHandle ,  (uc_sint32)milliseconds );
...
{
    Debug_System_print("[START] SemaphoreHandle:%lu \n" ,(uc_sint32)*pSemaphoreHandle );
...
uc_result UniversalClientSPI_Mutex_Open(uc_mutex_handle * pMutexHandle)
{
    ...
    */
    ...
    Debug_System_print("[END] Mutex_Open  MutexHandle:%ld\n" , *pMutexHandle );
...
{
    Debug_System_print("[START] Lock Mutex_Handle:%lu \n" , (uc_sint32)mutexHandle );
...
{
    Debug_System_print("[START] Unlock Mutex_handle:%lu \n" , (uc_sint32)mutexHandle );
...
{
    Debug_System_print("[START]Mutex_Close , MutexHandle:%lu\n" , (uc_sint32)*pMutexHandle);
    ...
    uc_thread_handle * pThreadHandle)
{
    ...
    if(s_tasknum==0)
    {
        Debug_System_print("[Set] Tasknum=0 reset all task-data \n");
    ...
    {
        Debug_System_print("[START] Index out of range!:%d \n", index);
        return UC_ERROR_TOO_MANY_HANDLES;
    }
#endif
    Debug_System_print("[START] Find First Index:%d \n", index);
    ...
    {
        Debug_System_print(">>>> UC_ERROR_OUT_OF_MEMORY\n");
    ...
    {
        Debug_System_print(">>>> UC_ERROR_UNABLE_TO_CREATE\n");
    ...
    Debug_System_print(">>>> taskID = %ld , TaskNum=:%u \n",  s_gucTask[index].iId , s_tasknum );
...
{
    Debug_System_print("[START]THandle:%ld   Delay:%d\n" , (uc_sint32)hThreadHandle , wDelay);
...
{
    Debug_System_print("[START] UniversalClientSPI_Thread_Close  ThreadHandle:%lu \n" ,(uc_sint32)*pThreadHandle );
    ...
    for(index=0 ; index < MAX_TASK_NUM ; index++ )
    {
        if( s_gucTask[index].iId==(uc_sint32)*pThreadHandle)
        {
            ...
            *(pThreadHandle)=(uc_thread_handle)(s_gucTask[index].iId);//is it meaningful?
            ...
            Debug_System_print("[Reset Free] UniversalClientSPI_Thread_Close Reset-task data index:%d \n" , index );
            break;
        }
    }

    s_tasknum--;
    Debug_System_print("[END] UniversalClientSPI_Thread_Close Tasknum:%u \n" ,s_tasknum );
    *pThreadHandle=NULL;
...
static void UniversalClient_PSTask(void)
{
	...
	#if 1
    Debug_print("[START] UniversalClient_PSTask\n");
	#endif
	...
	#if 1
	Debug_print("[END] UniversalClient_PSTask\n");
	#endif
...
uc_result UniversalClientSPI_PS_Delete(uc_uint32 index)
{
	...
	#if 1
	Debug_print("[START] UniversalClientSPI_PS_Delete idx:%d\n", index);
	#endif

	/* Check if PS is inited */
	if(NULL == s_ucAllPSBuffer)
	{
		Debug_print("[%s %d] UC_ERROR_SUCCESS\n", __FUNCTION__, __LINE__);
	...
	{
        Debug_print("[%s %d] UC_ERROR_NOT_IMPLEMENTED\n", __FUNCTION__, __LINE__);
	...
	if(bRet == FALSE)
	{
		Debug_print("[%s %d] UC_ERROR_OS_FAILURE\n", __FUNCTION__, __LINE__);
	...
	#if 1
	Debug_print("[END] UniversalClientSPI_PS_Delete\n");
	#endif
...
uc_result UniversalClientSPI_PS_Write(uc_uint32 index, uc_uint32 offset, const uc_buffer_st *pData)
{
	...
	#if 1
    Debug_print("[START] UniversalClientSPI_PS_Write idx:%d, offset:0x%x, length:0x%x\n", index, offset, pData->length);
	#endif
	...
	#if 1
	Debug_print("[END] UniversalClientSPI_PS_Write\n");
	#endif
...
uc_result UniversalClientSPI_PS_Read(uc_uint32 index, uc_uint32 offset, uc_buffer_st *pData)
{
	...
	#if 1
	Debug_print("[START] UniversalClientSPI_PS_Read idx:%d, offset:0x%x, length:0x%x\n", index, offset, pData->length);
	#endif

	/* Check if PS is inited */
	if(NULL == s_ucAllPSBuffer)
	{
		Debug_print("[%s %d] UC_ERROR_OUT_OF_MEMORY\n", __FUNCTION__, __LINE__);
 		return UC_ERROR_OUT_OF_MEMORY;
	}
	...
	if(pData == NULL)
	{
		Debug_print("[%s %d] UC_ERROR_NULL_PARAM\n", __FUNCTION__, __LINE__);
 		return UC_ERROR_NULL_PARAM;
	}
	...
	if(bRet == FALSE)
	{
		Debug_print("[%s %d] UC_ERROR_OS_FAILURE::index=%d, offset=%d\n", __FUNCTION__, __LINE__, index, offset);
 		return UC_ERROR_OS_FAILURE;
	}
	...
	if(bRet == FALSE)
	{
		Debug_print("[END] UniversalClientSPI_PS_Read::UC_ERROR_OS_FAILURE\n");
 		return UC_ERROR_OS_FAILURE;
	}

	#if 1
	Debug_print("[END] UniversalClientSPI_PS_Read::UC_ERROR_SUCCESS\n");
	#endif
...
uc_result UniversalClientSPI_PS_Initialize(void)
{
    ...
    Debug_print("[START] UniversalClientSPI_PS_Initialize\n");


	#if 1
	//DUMP THE SPI ARE
	Debug_print("\n\nCCA SPI AREA USAGE\n");
    Debug_print("_M_FLASH_CCA_DATA1_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_DATA1_ADDR_);
    Debug_print("_M_FLASH_CCA_SC_INIT_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_SC_INIT_ADDR_);
    Debug_print("_M_FLASH_CCA_SC1_INIT_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_SC1_INIT_ADDR_);
    Debug_print("_M_FLASH_CCA_SC_DNLD_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_SC_DNLD_ADDR_);
    Debug_print("_M_FLASH_CCA_SC1_DNLD_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_SC1_DNLD_ADDR_);
    Debug_print("_M_FLASH_CCA_CFG_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_CFG_ADDR_);
    Debug_print("_M_FLASH_CCA_SOFTCELL_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_SOFTCELL_ADDR_);
	Debug_print("CCA SPI AREA USAGE\n\n");
    #endif
    ...
    Debug_print("[END] UniversalClientSPI_PS_Initialize\n");
    return UC_ERROR_SUCCESS;
}
...
{
    Debug_print("[START] UniversalClientSPI_PS_Terminate\n");
    ...
    Debug_print("[END] UniversalClientSPI_PS_Terminate\n");
    return UC_ERROR_SUCCESS;
}
/** @} */ /* End of ps */
#else
...
uc_result UniversalClientSPI_PS_Delete(uc_uint32 index)
{
    Debug_print("[START] UniversalClientSPI_PS_Delete index 0x%x\n",index  );
    ...
    switch(index)
    {
		...
		case 2:
        {
			...
			Debug_print("NOT SUPPORT UniversalClientSPI_PS_Delete index 0x%x\n",index  );
			return UC_ERROR_NOT_IMPLEMENTED;
			#endif		
        ...
        default:
        {
            Debug_print("Unsupported index %d\n",index);
            return UC_ERROR_RESOURCE_NOT_FOUND;
        }
        break;
    }
    ...
    if(bRet == FALSE)
    {
        Debug_print("[END] UniversalClientSPI_PS_Deleta::UC_ERROR_OS_FAILURE\n");
        return UC_ERROR_OS_FAILURE;
    }
    return UC_ERROR_SUCCESS;
}
...
uc_result UniversalClientSPI_PS_Write(uc_uint32 index, uc_uint32 offset, const uc_buffer_st *pData)
{
    ...
    Debug_print("[START] UniversalClientSPI_PS_Write index 0x%x, offset 0x%x, pData 0x%x, length %d\n",index, offset, *(pData->bytes), pData->length   );
    uc_uint32 u32BaseAddr = 0;
    uc_bool bRet = FALSE;
    ...
    switch(index)
    {
        case 0:
        {
            ...
            {
                Debug_print("Write PS exceed partion size\n");
                return UC_ERROR_OUT_OF_MEMORY;
            }
        }
        break;
    ...
    if(bRet <= 0)
    {
        Debug_print("[END] UniversalClientSPI_PS_Write::UC_ERROR_OS_FAILURE\n");
        return UC_ERROR_OS_FAILURE;
    }
    Debug_print("[END] UniversalClientSPI_PS_Write \n"  );
    return UC_ERROR_SUCCESS;
...
uc_result UniversalClientSPI_PS_Read(uc_uint32 index, uc_uint32 offset, uc_buffer_st *pData)
{
    Debug_print("[START] UniversalClientSPI_PS_Read index 0x%x offset 0x%x length 0x%x\n",index,offset,pData->length  );
    uc_uint32 u32BaseAddr = 0;
    uc_bool bRet = FALSE;
    uc_uint8 *pu8TmpBuf = NULL;
    ...
    switch(index)
    {
        case 0:
        {
            ...
            if((offset+(pData->length)) > _M_FLASH_CCA_DATA1_SIZE_)
            {
                Debug_print("Read PS exceed partion size\n");
                return UC_ERROR_OUT_OF_MEMORY;
            }
        }
        break;
...
uc_result UniversalClientSPI_PS_GetProperty(uc_uint32 index, uc_ps_property *pProperty)
{
    ...
    Debug_print("[END] %s %d index[%d] reservedSize[%x]k\n",__func__,__LINE__,index,pProperty->reservedSize);
	return UC_ERROR_SUCCESS;
}
...
uc_result UniversalClientSPI_PS_Initialize(void)
{
    ...
    Debug_print("_M_FLASH_CCA_CFG_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_CFG_ADDR_);
    Debug_print("_M_FLASH_CCA_SOFTCELL_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_SOFTCELL_ADDR_);
    Debug_print("_M_FLASH_CCA_IFCP1_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_IFCP1_ADDR_);
    Debug_print("_M_FLASH_CCA_IFCP2_ADDR_ = %08lX \n", (uc_uint32)_M_FLASH_CCA_IFCP2_ADDR_);
	Debug_print("CCA SPI AREA USAGE\n\n");
    #endif
#if 0
...
uc_result UniversalClientSPI_Device_GetDeviceID(uc_buffer_st *pData)
{
    ...
    Debug_print(">>>>DeviceID::0x%02X 0x%02X 0x%02X 0x%02X\n",
                pData->bytes[0], pData->bytes[1], pData->bytes[2], pData->bytes[3]);

    Debug_print("[END] GetDeviceID\n");
...
{
    Debug_print("[START] GetCSSN setSecure[%d]\n" ,s_spi_ucSystem.setSecure);
...
//debug cssn
	...
	Debug_print(" >>>>GetCSSN:  end \n");
#endif
    Debug_print("[END] GetCSSN\n");
...
{
    Debug_print("[START] GetPrivateData \n"  );
...
#if 0
	uc_byte 		dataCSSN[5];
    memset(&dataCSSN[0], 0x00, 5);	


    if(pData == NULL || pData->bytes == NULL)
    {
        Debug_print("[END] GetPrivateData::UC_ERROR_NULL_PARAM\n");
        return UC_ERROR_NULL_PARAM;
    }

    if(s_spi_ucSystem.setPrivateData == UC_FALSE)
    {
        uc_result       Result = UC_ERROR_SUCCESS;
        uc_buffer_st    bfCSSN;

        bfCSSN.bytes = &dataCSSN[0];
        bfCSSN.length = 5;

        Result = UniversalClientSPI_Device_GetCSSN(&bfCSSN);

        if(Result == UC_ERROR_SUCCESS)
        {
			//to do
			// it needs to use the cssn to caculate the unique private data to use
        }


/*
        // Read private data from manufacture params partition(0x230000, 0x10000)
        manufacture_params_info stRead;
        _FLASH_Read(1, MP_PATITION_ADDR/2, (U16*)&stRead, sizeof(stRead)/2);
        memset(&s_spi_ucSystem.strPrivateData[0], 0x00, PRIVATE_DATA_SIZE);
        memcpy(&s_spi_ucSystem.strPrivateData[0], &stRead.ca_Privatedata[0], PRIVATE_DATA_SIZE);

        if(u16PriPerDataParam == 0)
        {
            printf(">>>>GetPrivateData=0x%02X%02X%02X%02X %02X%02X%02X%02X %02X%02X%02X%02X %02X%02X%02X%02X\n",
                    s_spi_ucSystem.strPrivateData[0], s_spi_ucSystem.strPrivateData[1], s_spi_ucSystem.strPrivateData[2], s_spi_ucSystem.strPrivateData[3],
                    s_spi_ucSystem.strPrivateData[4], s_spi_ucSystem.strPrivateData[5], s_spi_ucSystem.strPrivateData[6], s_spi_ucSystem.strPrivateData[7],
                    s_spi_ucSystem.strPrivateData[8], s_spi_ucSystem.strPrivateData[9], s_spi_ucSystem.strPrivateData[10], s_spi_ucSystem.strPrivateData[11],
                    s_spi_ucSystem.strPrivateData[12], s_spi_ucSystem.strPrivateData[13], s_spi_ucSystem.strPrivateData[14], s_spi_ucSystem.strPrivateData[15]);
        }
        s_spi_ucSystem.setPrivateData = UC_TRUE;
*/
    }


    printf("cssn dataCSSN[3] 0x%x\n",dataCSSN[3]);
    //for Kirin chip
	switch(dataCSSN[3])
	{
	    case 0x1:
        case 0x2:
        case 0x3:  //742BCD513DD639036112342FD031A208
 		s_spi_ucSystem.strPrivateData[15]=0x08;
		s_spi_ucSystem.strPrivateData[14]=0xA2;
		s_spi_ucSystem.strPrivateData[13]=0x31;
		s_spi_ucSystem.strPrivateData[12]=0xD0;
		s_spi_ucSystem.strPrivateData[11]=0x2F;
		s_spi_ucSystem.strPrivateData[10]=0x34;
		s_spi_ucSystem.strPrivateData[9]=0x12;
		s_spi_ucSystem.strPrivateData[8]=0x61;
		s_spi_ucSystem.strPrivateData[7]=0x03;
		s_spi_ucSystem.strPrivateData[6]=0x39;
		s_spi_ucSystem.strPrivateData[5]=0xD6;
		s_spi_ucSystem.strPrivateData[4]=0x3D;
		s_spi_ucSystem.strPrivateData[3]=0x51;
		s_spi_ucSystem.strPrivateData[2]=0xCD;
		s_spi_ucSystem.strPrivateData[1]=0x2B;
		s_spi_ucSystem.strPrivateData[0]=0x74;				
		break;           
        case 0x4:  //92C37CB6E90DC35594B18F69F7168C5B
		s_spi_ucSystem.strPrivateData[15]=0x5B;
		s_spi_ucSystem.strPrivateData[14]=0x8C;
		s_spi_ucSystem.strPrivateData[13]=0x16;
		s_spi_ucSystem.strPrivateData[12]=0xF7;
		s_spi_ucSystem.strPrivateData[11]=0x69;
		s_spi_ucSystem.strPrivateData[10]=0x8F;
		s_spi_ucSystem.strPrivateData[9]=0xB1;
		s_spi_ucSystem.strPrivateData[8]=0x94;
		s_spi_ucSystem.strPrivateData[7]=0x55;
		s_spi_ucSystem.strPrivateData[6]=0xC3;
		s_spi_ucSystem.strPrivateData[5]=0x0D;
		s_spi_ucSystem.strPrivateData[4]=0xE9;
		s_spi_ucSystem.strPrivateData[3]=0xB6;
		s_spi_ucSystem.strPrivateData[2]=0x7C;
		s_spi_ucSystem.strPrivateData[1]=0xC3;
		s_spi_ucSystem.strPrivateData[0]=0x92;				
		break; 
        case 0x5:  //D716AF228F8C494DA048D0955C122BC1
		s_spi_ucSystem.strPrivateData[15]=0xC1;
		s_spi_ucSystem.strPrivateData[14]=0x2B;
		s_spi_ucSystem.strPrivateData[13]=0x12;
		s_spi_ucSystem.strPrivateData[12]=0x5C;
		s_spi_ucSystem.strPrivateData[11]=0x95;
		s_spi_ucSystem.strPrivateData[10]=0xD0;
		s_spi_ucSystem.strPrivateData[9]=0x48;
		s_spi_ucSystem.strPrivateData[8]=0xA0;
		s_spi_ucSystem.strPrivateData[7]=0x4D;
		s_spi_ucSystem.strPrivateData[6]=0x49;
		s_spi_ucSystem.strPrivateData[5]=0x8C;
		s_spi_ucSystem.strPrivateData[4]=0x8F;
		s_spi_ucSystem.strPrivateData[3]=0x22;
		s_spi_ucSystem.strPrivateData[2]=0xAF;
		s_spi_ucSystem.strPrivateData[1]=0x16;
		s_spi_ucSystem.strPrivateData[0]=0xD7;				
		break;    
        case 0x6:  //6FCF86884A79A66EEB167C40A6252EE0
		s_spi_ucSystem.strPrivateData[15]=0xE0;
		s_spi_ucSystem.strPrivateData[14]=0x2E;
		s_spi_ucSystem.strPrivateData[13]=0x25;
		s_spi_ucSystem.strPrivateData[12]=0xA6;
		s_spi_ucSystem.strPrivateData[11]=0x40;
		s_spi_ucSystem.strPrivateData[10]=0x7C;
		s_spi_ucSystem.strPrivateData[9]=0x16;
		s_spi_ucSystem.strPrivateData[8]=0xEB;
		s_spi_ucSystem.strPrivateData[7]=0x6E;
		s_spi_ucSystem.strPrivateData[6]=0xA6;
		s_spi_ucSystem.strPrivateData[5]=0x79;
		s_spi_ucSystem.strPrivateData[4]=0x4A;
		s_spi_ucSystem.strPrivateData[3]=0x88;
		s_spi_ucSystem.strPrivateData[2]=0x86;
		s_spi_ucSystem.strPrivateData[1]=0xCF;
		s_spi_ucSystem.strPrivateData[0]=0x6F;				
		break; 
        default:
        break;
    ...
    #if 0// FOR MSTAR TEST BOARD
	switch(dataCSSN[3])
	{
		case 0x00://742BCD513DD639036112342FD031A208
		case 0xD7://742BCD513DD639036112342FD031A208
		s_spi_ucSystem.strPrivateData[15]=0x08;
		s_spi_ucSystem.strPrivateData[14]=0xA2;
		s_spi_ucSystem.strPrivateData[13]=0x31;
		s_spi_ucSystem.strPrivateData[12]=0xD0;
		...
		s_spi_ucSystem.strPrivateData[0]=0x74;				
		break;
		case 0x6E: //92C37CB6E90DC35594B18F69F7168C5B
		case 0xCC: //92C37CB6E90DC35594B18F69F7168C5B
		s_spi_ucSystem.strPrivateData[15]=0x5B;
		s_spi_ucSystem.strPrivateData[14]=0x8C;
		s_spi_ucSystem.strPrivateData[13]=0x16;
		s_spi_ucSystem.strPrivateData[12]=0xF7;
		...
		s_spi_ucSystem.strPrivateData[0]=0x92;				
		break;
		case 0x0A: //D716AF228F8C494DA048D0955C122BC1
		case 0xD1: //D716AF228F8C494DA048D0955C122BC1
		s_spi_ucSystem.strPrivateData[15]=0xC1;
		s_spi_ucSystem.strPrivateData[14]=0x2B;
		s_spi_ucSystem.strPrivateData[13]=0x12;
		s_spi_ucSystem.strPrivateData[12]=0x5C;
		...
		s_spi_ucSystem.strPrivateData[0]=0xD7;			
		break;
		case 0x0B: //6FCF86884A79A66EEB167C40A6252EE0
		case 0xCB: //6FCF86884A79A66EEB167C40A6252EE0
		s_spi_ucSystem.strPrivateData[15]=0xE0;
		s_spi_ucSystem.strPrivateData[14]=0x2E;
		s_spi_ucSystem.strPrivateData[13]=0x25;
		s_spi_ucSystem.strPrivateData[12]=0xA6;
		...
		s_spi_ucSystem.strPrivateData[0]=0x6F;					
		break;
		case 0x09: //50C4D354F02CB0827F45AE37C0FE445E
		case 0xC2: //50C4D354F02CB0827F45AE37C0FE445E
		s_spi_ucSystem.strPrivateData[15]=0x5E;
		s_spi_ucSystem.strPrivateData[14]=0x44;
		s_spi_ucSystem.strPrivateData[13]=0xFE;
		s_spi_ucSystem.strPrivateData[12]=0xC0;
		s_spi_ucSystem.strPrivateData[11]=0x37;
		s_spi_ucSystem.strPrivateData[10]=0xAE;
		s_spi_ucSystem.strPrivateData[9]=0x45;
		s_spi_ucSystem.strPrivateData[8]=0x7F;
		s_spi_ucSystem.strPrivateData[7]=0x82;
		s_spi_ucSystem.strPrivateData[6]=0xB0;
		s_spi_ucSystem.strPrivateData[5]=0x2C;
		s_spi_ucSystem.strPrivateData[4]=0xF0;
		s_spi_ucSystem.strPrivateData[3]=0x54;
		s_spi_ucSystem.strPrivateData[2]=0xD3;
		s_spi_ucSystem.strPrivateData[1]=0xC4;
		s_spi_ucSystem.strPrivateData[0]=0x50;				
		break;
		case 0xC8: //A0DF882F9557DFE515FB4EA40765A915
		s_spi_ucSystem.strPrivateData[15]=0x15;
		s_spi_ucSystem.strPrivateData[14]=0xA9;
		s_spi_ucSystem.strPrivateData[13]=0x65;
		s_spi_ucSystem.strPrivateData[12]=0x07;
		s_spi_ucSystem.strPrivateData[11]=0xA4;
		s_spi_ucSystem.strPrivateData[10]=0x4E;
		s_spi_ucSystem.strPrivateData[9]=0xFB;
		s_spi_ucSystem.strPrivateData[8]=0x15;
		s_spi_ucSystem.strPrivateData[7]=0xE5;
		s_spi_ucSystem.strPrivateData[6]=0xDF;
		s_spi_ucSystem.strPrivateData[5]=0x57;
		s_spi_ucSystem.strPrivateData[4]=0x95;
		s_spi_ucSystem.strPrivateData[3]=0x2F;
		s_spi_ucSystem.strPrivateData[2]=0x88;
		s_spi_ucSystem.strPrivateData[1]=0xDF;
		s_spi_ucSystem.strPrivateData[0]=0xA0;	
		break;
		default: // 3554A075373D53156A92A0E13FB0CA14   for cssn is 0x00000000
		s_spi_ucSystem.strPrivateData[15]=0x14;
		s_spi_ucSystem.strPrivateData[14]=0xCA;
		s_spi_ucSystem.strPrivateData[13]=0xB0;
		s_spi_ucSystem.strPrivateData[12]=0x3F;
		s_spi_ucSystem.strPrivateData[11]=0xE1;
		s_spi_ucSystem.strPrivateData[10]=0xA0;
		s_spi_ucSystem.strPrivateData[9]=0x92;
		s_spi_ucSystem.strPrivateData[8]=0x6A;
		s_spi_ucSystem.strPrivateData[7]=0x15;
		s_spi_ucSystem.strPrivateData[6]=0x53;
		s_spi_ucSystem.strPrivateData[5]=0x3D;
		s_spi_ucSystem.strPrivateData[4]=0x37;
		s_spi_ucSystem.strPrivateData[3]=0x75;
		s_spi_ucSystem.strPrivateData[2]=0xA0;
		s_spi_ucSystem.strPrivateData[1]=0x54;
		s_spi_ucSystem.strPrivateData[0]=0x35;
		break;
	}
    #endif

    pData->length = PRIVATE_DATA_SIZE;

    Debug_print(" pData->length=%d\n", pData->length);

    memset(&pData->bytes[0], 0x00, PRIVATE_DATA_SIZE);
    memcpy(&pData->bytes[0], &s_spi_ucSystem.strPrivateData[0], PRIVATE_DATA_SIZE);


	Debug_print(">>>>GetPrivateData=0x%02X%02X%02X%02X %02X%02X%02X%02X %02X%02X%02X%02X %02X%02X%02X%02X\n",
			s_spi_ucSystem.strPrivateData[0], s_spi_ucSystem.strPrivateData[1], s_spi_ucSystem.strPrivateData[2], s_spi_ucSystem.strPrivateData[3],
			s_spi_ucSystem.strPrivateData[4], s_spi_ucSystem.strPrivateData[5], s_spi_ucSystem.strPrivateData[6], s_spi_ucSystem.strPrivateData[7],
			s_spi_ucSystem.strPrivateData[8], s_spi_ucSystem.strPrivateData[9], s_spi_ucSystem.strPrivateData[10], s_spi_ucSystem.strPrivateData[11],
			s_spi_ucSystem.strPrivateData[12], s_spi_ucSystem.strPrivateData[13], s_spi_ucSystem.strPrivateData[14], s_spi_ucSystem.strPrivateData[15]);
    ...
    Debug_print("[END] UniversalClientSPI_Device_GetPrivateData\n");
#endif
    return UC_ERROR_SUCCESS;
}
/**
...
{
    Debug_print("[START] SetCSSK \n"  );
#if 0
    if(pKeyMaterial == NULL || pKeyMaterial->bytes == NULL)
    {
        Debug_print( "SetCSSK::UC_ERROR_NULL_PARAM\n" );
        return UC_ERROR_NULL_PARAM;
    }

    if(pKeyMaterial->length == 0 || pKeyMaterial->length > CSSK_DATA_SIZE )
    {
        Debug_print(" SetCSSK::UC_ERROR_INVALID_LENGTH\n");
        return UC_ERROR_INVALID_LENGTH;
    }

    Debug_print(" pKeyMaterial->length=%d\n", pKeyMaterial->length);

    unsigned char       ucLength = CSSK_DATA_SIZE;

    if(pKeyMaterial->length < CSSK_DATA_SIZE)
    {
        ucLength = (unsigned char)pKeyMaterial->length;
    }

    s_spi_ucSystem.setCSSK = UC_TRUE;
    s_spi_ucSystem.typeCSSK = UC_DK_PROTECTION_UNKNOWN;

    /*
      Debug_print("pKeyMaterial->bytes= %02X %02X %02X %02X %02X %02X %02X %02X\n",
              *(pKeyMaterial->bytes+0), *(pKeyMaterial->bytes+1), *(pKeyMaterial->bytes+2), *(pKeyMaterial->bytes+3),
              *(pKeyMaterial->bytes+4), *(pKeyMaterial->bytes+5), *(pKeyMaterial->bytes+6), *(pKeyMaterial->bytes+7));
      */
    memset(&s_spi_ucSystem.strCSSK[0], 0x00, sizeof(&s_spi_ucSystem.strCSSK[0]));
    memcpy(&s_spi_ucSystem.strCSSK[0], pKeyMaterial->bytes, ucLength);

    printf(" pKeyMaterial->bytes= %02X %02X %02X %02X %02X %02X %02X %02X\n",
...
{
    Debug_print("[START] SetEdCSSK pCSSK->length:0x%x\n" ,pCSSKInfo->pCSSK->length);
#if 1
...
#if 1//UC_SPI_DEBUG
    printf(" pKeyMaterial->bytes= %02X %02X %02X %02X %02X %02X %02X %02X  %02X %02X %02X %02X %02X %02X %02X %02X\n",
                s_spi_ucSystem.strCSSK[0], s_spi_ucSystem.strCSSK[1], s_spi_ucSystem.strCSSK[2], s_spi_ucSystem.strCSSK[3],\
                s_spi_ucSystem.strCSSK[4], s_spi_ucSystem.strCSSK[5], s_spi_ucSystem.strCSSK[6], s_spi_ucSystem.strCSSK[7],\
...
#endif
    Debug_print("[END] SetEdCSSK  KeyProtection type:%d pCSSK->length:%d \n" ,  s_spi_ucSystem.typeCSSK,pCSSKInfo->pCSSK->length);
#endif
    return UC_ERROR_SUCCESS;
}
/**
...
{
    Debug_print("[START] GetSecurityState \n"  );
#if 1
    ...
    Debug_print("[END] GetSecurityState\n");
#endif
    return UC_ERROR_SUCCESS;
}
...
{
    Debug_print("\33[35m[START] GetPVRSecurityState \33[m\n"  );
...
{
    Debug_print("[START] GetPlatformIdentifiers \n"  );
...
{
    Debug_print("[START] GetPersonalizedData \n"  );
#if 0

    if( pData == NULL || pData->bytes == NULL )
    {
        Debug_print("[END] UC_ERROR_NULL_PARAM\n"  );
        return UC_ERROR_NULL_PARAM;
    }
    ...
    {
        Debug_print("GetIPAddress::UC_ERROR_INVALID_LENGTH\n");
        return UC_ERROR_INVALID_LENGTH;
    }*/
...
{
    Debug_print("[START] GetIPAddress \n"  );
    ...
    pData->bytes[0] = ipaddress & 0xFF;
    pData->bytes[1] = (ipaddress >> 8) & 0xFF;
    pData->bytes[2] = (ipaddress >> 16) & 0xFF;
    pData->bytes[3] = (ipaddress >> 24) & 0xFF;

    Debug_print("IP:: %d.%d.%d.%d\n", pData->bytes[3], pData->bytes[2], pData->bytes[1], pData->bytes[0]);
...
{
    Debug_print("[START] SetSessionKey \n"  );
#if 0
    ...
    Debug_print(" streamHandle=0x%08X  PVESKeyLength:%d \n" , streamHandle , pPVRSessionKey->length );
    ...
    Debug_print(" pPVRSessionKey->bytes= %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n",
...
{
    Debug_print("[START] SetExtendedSessionKey\n"  );
#if 0
...
//.0428    UniversalClient_PVR_setSessionKey(streamHandle, pPVRSKInfo->pPVRSK);
    ...
    Debug_print("[END] SetExtendedSessionKey\n"  );
#endif
    return UC_ERROR_SUCCESS;
}
/** @} */ /* End of pvrspi */
...
{
    Debug_print("streamHandle: %x \n", streamHandle);
	return  _UniversalClientSPI_Stream_Start(streamHandle);   
}
/**
...
//return UC_ERROR_SUCCESS;
	retVal = _UniversalClientSPI_Stream_Stop(streamHandle);
    Debug_print("ret: %d streamHandle: %x\n", retVal, streamHandle);
...
{
    uc_result retVal = UC_ERROR_SUCCESS;
	retVal = _UniversalClientSPI_Stream_OpenFilter(streamHandle,pFilterHandle);    
    Debug_print("ret: %d streamHandle: %x\n", retVal, streamHandle);
...
{
	Debug_print("filterHandle: %x \n", filterHandle);
    return _UniversalClientSPI_Stream_SetFilter(filterHandle,pFilterRules);
}
/**
    ...
    uc_filter_handle *pFilterHandle)
{
    ...
    Debug_print("ret: %d filterHandle: %d streamHandle: 0x%x \n", retVal, *pFilterHandle,streamHandle);
	//return UC_ERROR_SUCCESS;
	retVal = _UniversalClientSPI_Stream_CloseFilter(streamHandle,pFilterHandle);
    return retVal;

}

	//ca callback function MX
void ca_get_section_callbak(int pridata, char *buf, unsigned int  len)
{
	uc_stream_handle streamHandle = pridata;
	...
	else if((streamHandle&STREAM_TYPE_MASK) == ((uc_stream_handle)EMM_STREAM_HANDLE&STREAM_TYPE_MASK))
	{
	   ...
	   else
	   {
		   ...
		   Debug_print("streamHandle: 0x%x len: %d connectionHandle: %d\n", pridata, len,gEMMStreamInfo.connectionHandle[idx]);
		#if 0
			int j= 0;
		   for(j=0;j<len;j++)
		   {
	...
	else if((streamHandle&STREAM_TYPE_MASK) == ((uc_stream_handle)ECM_STREAM_HANDLE&STREAM_TYPE_MASK))
	{
	   ...
	   else
	   {
		   ...
		   gECMStreamInfo.notifyCallback[i](gECMStreamInfo.connectionHandle[i],&gECMStreamInfo.byteBufferStruct[i]);
		   Debug_print("streamHandle: 0x%x len: %d connectionHandle: %d\n", pridata, len,gECMStreamInfo.connectionHandle[i]);
#if 0
		   int k= 0;
		   for(k=0;k<len;k++)
		   {
...
{
    uc_result retVal = UC_ERROR_SUCCESS;
	Debug_print("ret: %d streamHandle: %x connectionHandle: %d\n", retVal, streamHandle,connectionHandle);
...
{
    uc_result retVal = UC_ERROR_SUCCESS;
	Debug_print("ret: %d streamHandle: 0x%x connectionHandle: %d\n", retVal, streamHandle,connectionHandle);
    ...
    uc_uint32 keyVersion)
{
   ...
   // DSCMB_Key_Type eKeyType = 0;
    //DSCMB_Type DscmbType = E_DSCMB_TYPE_CSA;
...
printf("YJC %s %d\n",__func__,__LINE__);
    ...
    //Current_StreamHandle = streamHandle;
    if(pKeyInfo == NULL)
    {
        retVal = UC_ERROR_NULL_PARAM;
    }
    /*else if(pKeyInfo->pDescramblingKey->length != 8)
    {
        retVal = UC_ERROR_NOT_IMPLEMENTED;
    }*/
    else
                ...
                for(j = 0; j < MAX_ECM_COMPONENT_NUM; j++)
                {
                    if(gECMStreamInfo.componentPid[i][j] != CCA_INVALID_PID)
                    {
                                ...
                                || pKeyInfo->descramblingKeyProtection == UC_DK_PROTECTION_UNKNOWN)
                        {
							...
							Debug_print("pDescramblingKey->length %x\n",pKeyInfo->pDescramblingKey->length);
							if(0==keyVersion)//evenkey
							{
							...
							if(retVal < 0)
							{
								Debug_print("GxDescSetCw error %x\n",retVal);
								DESCR_UNLOCK();
								return UC_ERROR_INVALID_LENGTH;
							}
...
uc_result  UniversalClientSPI_CopyControl_Macrovision_SetConfig(uc_macrovision_config mac_config_data)
{
    ...
    Debug_print("[END] UniversalClientSPI_CopyControl_Macrovision_SetConfig %d \n",ret);
#endif
    return UC_ERROR_SUCCESS;
}
/**
...
uc_result  UniversalClientSPI_CopyControl_Macrovision_SetMode(uc_stream_handle streamHandle, uc_macrovision_mode mac_mode)
{
    ...
    Debug_print("\n");
    Debug_print("[START] UniversalClientSPI_CopyControl_Macrovision_SetMode\n");
    Debug_print("[%s %d] streamHandle=0x%08X\n", __FUNCTION__, __LINE__, streamHandle);
    Debug_print("[%s %d] mac_mode=%d\n", __FUNCTION__, __LINE__, mac_mode.mode);
#if 0
    g_ucCCI.mvMode = mac_mode.mode;

    ret = UniversalClient_CCI_Macrovision(2);

    Debug_print("[END] UniversalClientSPI_CopyControl_Macrovision_SetMode %d\n",ret);
#endif
    return UC_ERROR_SUCCESS;
}
/**
...
uc_result  UniversalClientSPI_CopyControl_SetCCI(uc_stream_handle streamHandle, uc_copy_control_info* pCopyControlInfo)
{
    ...
    Debug_print("\n");
    Debug_print("[START] UniversalClientSPI_CopyControl_SetCCI\n");
    Debug_print("[START] Disable NOW!!\n");
    return UC_ERROR_SUCCESS;
#if 0
    if(pCopyControlInfo == NULL)
    {
        Debug_print("[END] UniversalClientSPI_CopyControl_SetCCI with UC_ERROR_NULL_PARAM\n");
        return UC_ERROR_NULL_PARAM;
    }

    Debug_print("[%s %d] streamHandle=0x%08X\n", __FUNCTION__, __LINE__, streamHandle);
    Debug_print("[%s %d] protocol_version=0x%02X\n", __FUNCTION__, __LINE__, pCopyControlInfo->protocol_version);
    Debug_print("[%s %d] aps_copy_control_info=0x%02X\n", __FUNCTION__, __LINE__, pCopyControlInfo->aps_copy_control_info);
    Debug_print("[%s %d] emi_copy_control_info=0x%02X\n", __FUNCTION__, __LINE__, pCopyControlInfo->emi_copy_control_info);
    Debug_print("[%s %d] ict_copy_control_info=0x%02X\n", __FUNCTION__, __LINE__, pCopyControlInfo->ict_copy_control_info);
    Debug_print("[%s %d] rct_copy_control_info=0x%02X\n", __FUNCTION__, __LINE__, pCopyControlInfo->rct_copy_control_info);
    Debug_print("[%s %d] rl_copy_control_info=0x%02X\n", __FUNCTION__, __LINE__, pCopyControlInfo->rl_copy_control_info);
    ...
    Debug_print("[END] UniversalClientSPI_CopyControl_SetCCI\n");
    ...
    // junchi.lin 20140508: avoid crash
        ...
        switch (Sc_Info.uProtocol)
        {
            ...
            case T0_PROTOCOL:
            {
                ...
                if (result != E_SC_OK)
                {
                    //S3VDSCDBG__(5, Debug_print("retry MApi_SC_T0_Communication\n\n"));
                    result = MApi_SC_T0_Communication(smartcardID, (uc_uint8*)pSendBuffer, &wSendDataLen , abReadData, &wReadDataLen);
                }
                ...
                {
//					printf("result %d\n",result);
                    //S3VDSCDBG__(0, Debug_print("    MApi_SC_T0_Communication FAIL %d\n",result));
                }
            ...
            case T14_PROTOCOL:
            {
                ...
                if (result != E_SC_OK)
                {
                    //S3VDSCDBG__(5, Debug_print("retry MApi_SC_T14_Communication\n\n"))
                    result = MApi_SC_T14_Communication(smartcardID, (uc_uint8*)pSendBuffer, &wSendDataLen , abReadData, &wReadDataLen);
                }
                if (result == E_SC_OK)
                {
...
{
    Debug_print("[START] UniversalClientSPI_DateTime_GetTimeOfDay\n");
#if 0
    ...
    //Debug_print("[END] UniversalClientSPI_DateTime_GetTimeOfDay\n");
#endif
    return UC_ERROR_SUCCESS;
}
/** @} */ /* End of datetimespi */
...
{
    Debug_print("[START] Crypto_Verify\n");
...
static void _TimerCallback(uc_uint32 u32StTimer, uc_uint32 u32TimerID)
{
    ...
    for(index=0 ; index<MAX_TASK_NUM ; index++ )
    {
        ...
        {
            Debug_print("[Match] find index:%d \n" , index );
            break;
        }
    }
    ...
    if(index==MAX_TASK_NUM)
    {
        Debug_print("[Match] Can not find match index . Return! \n" );
        return;
    }
...
{
	Debug_print("[start] UniversalClientSPI_Timer_Open\n");
#if 0	
    if(pTimerId == NULL||pTimerInfo==NULL||pTimerInfo->onTimeCallback==NULL)
    {
        Debug_print("[START]  UC_ERROR_NULL_PARAM \n");
        return UC_ERROR_NULL_PARAM;
    }
    ...
    if(s_timernum==0)
    {
        Debug_print("[Set] s_timernum=0 reset all timer-data \n");
        for(index=0 ; index<MAX_TASK_NUM ; index++ )
        {
    ...
    if(index>=MAX_TASK_NUM)
    {
        Debug_print("[START] Index out of range! Index:%d \n", index);
        return UC_ERROR_TOO_MANY_HANDLES;
    }

    Debug_print("[START] Find First Index:%d TimerInfo timeout:%u  IsRepetitive:%d  \n", index, pTimerInfo->timeout , pTimerInfo->isRepetitive );
    ...
    if(s32TimerId  < 0)
    {
        Debug_print("[End] Timer_Open fail! \n");
        return UC_ERROR_DRIVER_FAILURE;
    }
    ...
    *pTimerId=s32TimerId;
    ...
    Debug_print("[End]  Timer_Open TimerID:%lu ! timenum:%d \n" ,
...
uc_result UniversalClientSPI_Timer_Close  ( uc_uint32  timerId  )
{
    ...
    if(Rest==FALSE)
    {
        Debug_print("[ERROR] MsOS_DeleteTimer FALSE! \n"  );
        return UC_ERROR_DRIVER_FAILURE;
    }
    ...
    Debug_print("[End] TimerID:%u  Index:%d  timerNum:%d\n" , timerId ,index , s_timernum);
#endif
    return UC_ERROR_SUCCESS;
...
uc_result UniversalClientSPI_Timer_Stop  ( uc_uint32  timerId  )
{
    ...
    if(Rest==FALSE)
    {
        Debug_print("[ERROR] MsOS_StopTimer FALSE!\n" );
        return UC_ERROR_DRIVER_FAILURE;
    }
#endif
    return UC_ERROR_SUCCESS;
...
uc_result UniversalClientSPI_Timer_Start  ( uc_uint32  timerId  )
{
    ...
    if(Rest==FALSE)
    {
        Debug_print("[ERROR] MsOS_StartTimer FALSE! \n"  );
        return UC_ERROR_DRIVER_FAILURE;
    }
#endif
    return UC_ERROR_SUCCESS;
...
void GL_DESCR_Init(void)
{
	...
	if(0 == DESCR_Info.MutexHandle)
	{
		...
		{
			Debug_print("  UniversalClientSPI_Mutex_Open failed 0x%x \n", DESCR_Info.MutexHandle);
			return UC_ERROR_OS_FAILURE;
		}
	}
	...
	Debug_print(" GxDescInit Ret =%d",Ret);
	if(Ret < 0)
	{
		printf("GxDescInit error\n");
		return ;
	}
	#if 0
	Ret =0;
	...
	Debug_print(" GxFuse_GetCwDualModeDisableStatus Ret =%d",Ret);
	Ret = GxFuse_GetMSR2KlmCWStage();
	Debug_print(" GxFuse_GetMSR2KlmCWStage Ret =%d",Ret);
	Ret = GxFuse_GetAlgDisableStatus();
	Debug_print(" GxFuse_GetAlgDisableStatus Ret =%d",Ret);
#endif
#if 0
...
uc_result UniversalClientSPI_IFCP_Communicate(uc_IFCP_input *pInput, uc_IFCP_output *pOutput)
{
    ...
    if(pInput == NULL)
    {
        printf("[%s][%d] invaild input buffer\n",__func__,__LINE__);
        return UC_ERROR_INSUFFICIENT_BUFFER;
    }
	...
	//Debug_print("[START] 0x40, 0x10, 0x03?? == 0x%x 0x%x 0x%x 0x%x 0x%x\n",pInput->pIFCPInfo->pKLCInfo->header[0],pInput->pIFCPInfo->pKLCInfo->header[1],pInput->pIFCPInfo->pKLCInfo->header[2],pInput->pIFCPInfo->pKLCInfo->header[3],pInput->pIFCPInfo->pKLCInfo->header[4]);  
    _param.ifcp_header_len = 3;                      // 必定是3个字节
    _param.ifcp_payload = (pInput->pIFCPInfo->pKLCInfo->payload).bytes;                             // CA库给的
    _param.ifcp_payload_len = (pInput->pIFCPInfo->pKLCInfo->payload).length;                         // CA库给的
	if(NULL!=(void *)(pInput->pIFCPInfo->pApplicationControlInfo))//->payload)
	{		
		_param.ifcp_app_header = pInput->pIFCPInfo->pApplicationControlInfo->header;						  // CA库给的
		_param.ifcp_app_header_len = IFCP_APP_CONTROL_INFO_HEADER_LEN;						// CA库给的，没有的时候需要填0
		_param.ifcp_app_payload = (pInput->pIFCPInfo->pApplicationControlInfo->payload).bytes;						  // CA库给的
	...
	else
	{
		...
		_param.ifcp_app_payload = NULL;						  // CA库给的
    ...
    
{
    ...
    Debug_print("streamHandle: %x connectionHandle %d\n", streamHandle, connectionHandle);
