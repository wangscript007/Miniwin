	...
	const uc_elementary_component *pComponent)
{
	...
	{
		NGLOG_DEBUG("[%s %d] Stream_AddComponent for PVR RECORD\n", __FUNCTION__, __LINE__);
	}
	else if(streamHandle == gPVRStreamInfo.pvrPlaybackStreamHandle)
	{
		NGLOG_DEBUG("[%s %d] Stream_AddComponent for PVR PLAYBACK\n", __FUNCTION__, __LINE__);
	}
#endif
	else
	{
      ...
      const uc_elementary_component *pComponent)
{
	...
	NGLOG_DEBUG("streamHandle: %x componentPid: 0x%x \n", streamHandle, pComponent->componentStream.pid);
#if 1
	if(pComponent == NULL)
	{
		retVal = UC_ERROR_NULL_PARAM;
	...
	{
		NGLOG_DEBUG("[%s %d] Stream_RemoveComponent for PVR RECORD\n", __FUNCTION__, __LINE__);
	}
	else if(streamHandle == gPVRStreamInfo.pvrPlaybackStreamHandle)
	{
		NGLOG_DEBUG("[%s %d] Stream_RemoveComponent for PVR PLAYBACK\n", __FUNCTION__, __LINE__);
	}
#endif
    else
    {
            ...
            else
            {
                ...
                if(pstreamComponent->componentStream.protocolType == UC_STREAM_DVB)
                {
                    ...
                    else
                    {
                        ...
                        if(retDscmb == FALSE)
                        {
                            NGLOG_ERROR("MDrv_DSCMB_FltDisconnectPid err\n");
                        }
