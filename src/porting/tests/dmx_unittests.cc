...
TEST_F(DMX,Alloc_Max){
   ...
   for(int i=0;i<MAX_FILTERS/2;i++)
       ASSERT_EQ(nglFreeSectionFilter(flts[i]),NGL_OK);
}
...
TEST_F(DMX,NullFilter){
   ASSERT_NE(NGL_OK,nglStartSectionFilter(0));
   ASSERT_NE(NGL_OK,nglSetSectionFilterParameters(NULL,NULL,NULL,0));
   ASSERT_NE(NGL_OK,nglStopSectionFilter(0));
   ASSERT_NE(NGL_OK,nglFreeSectionFilter(0));
}
...
TEST_F(DMX,SetFilterParam_1){
   ...
   ASSERT_EQ(NGL_OK,nglStopSectionFilter(flt));
   nglFreeSectionFilter(flt);
}
...
TEST_F(DMX,TDT){
   ...
   ASSERT_EQ(0,nglSetSectionFilterParameters(flt,mask,value,1));
   ASSERT_EQ(0,nglStartSectionFilter(flt));
   ASSERT_EQ(NGL_OK,nglWaitEvent(eventHandle,20000));
   ASSERT_EQ(0,nglStopSectionFilter(flt));
   nglFreeSectionFilter(flt);
}
...
TEST_F(DMX,Filter_MASK_3){
   ...
   ASSERT_EQ(GetSection(0,0,PAT),NGL_OK);
