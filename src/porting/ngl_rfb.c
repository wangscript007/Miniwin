...
static void onMousePtr(int buttonMask,int x,int y,rfbClientPtr cl){
   ...
   if(x>=0 && y>=0 && x<rfbScreen->width && y<rfbScreen->height) {
      ...
      if(btn>0&&btn<0x10){
          NGLOG_VERBOSE("ButtonMask %x->%x btn=%x/%d",cd->oldButton,buttonMask,btn,btnidx(btn),btns[btnidx(btn)]);
...
{
     NGLOG_VERBOSE("rcv KEY %d down=%d",key,down);
     switch(key){
     case XK_F1:SENDKEY(KEY_RED,down);break;
     case XK_F2:SENDKEY(KEY_GREEN,down);break;
     case XK_F3:SENDKEY(KEY_YELLOW,down);break;
     ...
     case XK_0     :SENDKEY(KEY_0,down);NGLOG_DEBUG("0");break;
     ...
     case XK_K     :SENDKEY(KEY_K,down);NGLOG_DEBUG("K");break;
...
#define PRINTFMT(f) NGLOG_VERBOSE("truecolor=%d ,depth=%d bitsPerPixel=%d rgbmax=%x/%x/%x rgbshift=%x/%x/%x",\
...
DWORD nglGetSurfaceInfo(HANDLE surface,UINT*width,UINT*height,INT *format)
{
    ...
    //NGLOG_VERBOSE("%p size=(%dx%d) format=%d",ngs,ngs->width,ngs->height,ngs->format);
    //PRINTFMT(rfbScreen);
    return NGL_OK;
}
...
DWORD nglFlip(HANDLE surface){
    ...
    NGLOG_VERBOSE("flip %p ishw=%d",ngs,ngs->ishw);
    return 0;
}
...
{//XShmCreateImage XShmCreatePixmap
     ...
     if(format!=GPF_ABGR&&format!=GPF_ARGB){
         NGLOG_DEBUG("Format %d is not supported %d",format,GPF_ARGB);
     ...
     *surface=(HANDLE)nglsurface;
     surface_count++;
     NGLOG_DEBUG("surface=%p framebuffer=%p size=%dx%d format=%d hw=%d  surfacecount=%d",nglsurface,
...
DWORD nglBlit(HANDLE dstsurface,int dx,int dy,HANDLE srcsurface,const NGLRect*srcrect)
{
     ...
     if(((int)rs.w+dx<=0)||((int)rs.h+dy<=0)||(dx>=(int)ndst->width)||(dy>=(int)ndst->height)||(rs.x<0)||(rs.y<0)){
         NGLOG_VERBOSE("dx=%d,dy=%d rs=(%d,%d-%d,%d)",dx,dy,rs.x,rs.y,rs.w,rs.h);
         return NGL_INVALID_PARA;
     }

     NGLOG_VERBOSE("Blit %p[%dx%d] %d,%d-%d,%d -> %p[%dx%d] %d,%d",nsrc,nsrc->width,nsrc->height,
     ...
     NGLOG_VERBOSE("Blit %p %d,%d-%d,%d -> %p %d,%d buffer=%p->%p",nsrc,rs.x,rs.y,rs.w,rs.h,ndst,dx,dy,pbs,pbd);
...
DWORD nglDestroySurface(HANDLE surface)
{
     ...
     surface_count--;
     NGLOG_DEBUG("Surface %p Destroied hw=%d surfacecount=%d",ngs,ngs->ishw,surface_count);
     free(ngs);
     return 0;
}
