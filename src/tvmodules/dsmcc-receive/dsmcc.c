...
{
	/* assert */
	if(dsi->compatibilityDescriptor.compatibilityDescriptorLength != 0)//if compatibilityDescriptorLength==0 GroupInfoIndication is here
		fatal("Unexpected DSI format");

	return ((unsigned char *) dsi) + sizeof(struct DownloadServerInitiate);
}
unsigned char*DSI_groupInfoIndication(struct GroupInfoIndication*grp){
