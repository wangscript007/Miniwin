...
bool PathInfo::match(const char *path)
{
    ...
    case prefix:
    {
        ...
        if (!warned && ret && (strlen(pattern) != strlen(path))) {
            NGLOG_WARN("Deprecated implied prefix pattern detected, please use '%s*' instead", pattern);
            warned = true;
        }
        return ret;
    }
...
DirectVolume::DirectVolume(VolumeManager *vm, const fstab_rec*rec, int flags) :
    ...
    if (strcmp(rec->mount_point, "auto") != 0) {
        NGLOG_ERROR("Vold managed volumes must have auto mount point; ignoring %s",rec->mount_point);
    }
        ...
        if ((*it)->match(dp)) {
            ...
            if (action == NetlinkEvent::NlActionAdd) {
                ...
                if (createDeviceNode(nodepath, major, minor)) {
                    NGLOG_ERROR("Error making device node '%s' (%s)", nodepath, strerror(errno));
                }
                ...
                if (getState() == Volume::State_Idle) {
                    ...
                    snprintf(msg, sizeof(msg), "Volume %s %s disk inserted (%d:%d)", getLabel(), getFuseMountpoint(), mDiskMajor, mDiskMinor);
                ...
                if (!strcmp(devtype, "disk")) {
                    ...
                    NGLOG_WARN("Ignoring non add/remove/change event");
            }
...
void DirectVolume::handleDiskAdded(const char * /*devpath*/,
    ...
    if (tmp) {
        ...
        NGLOG_WARN("Kernel block uevent missing 'NPARTS'");
        mDiskNumParts = 1;
    }
    ...
    if (mDiskNumParts == 0) {
        ...
        NGLOG_DEBUG("Dv::diskIns - No partitions - good to go son!");
#endif
        setState(Volume::State_Idle);
    } else {
#ifdef PARTITION_DEBUG
        NGLOG_DEBUG("Dv::diskIns - waiting for %d pending partitions", mPendingPartCount);
#endif
...
void DirectVolume::handlePartitionAdded(const char *devpath, NetlinkEvent *evt) {
    ...
    if (tmp) {
        ...
        NGLOG_WARN("Kernel block uevent missing 'PARTN'");
        part_num = 1;
    }

    if (part_num > MAX_PARTITIONS || part_num < 1) {
        NGLOG_ERROR("Invalid 'PARTN' value");
        return;
    }
    ...
    if (major != mDiskMajor) {
        NGLOG_ERROR("Partition '%s' has a different major than its disk!", devpath);
        return;
    }
#ifdef PARTITION_DEBUG
    NGLOG_DEBUG("Dv:partAdd: part_num = %d, minor = %d\n", part_num, minor);
#endif
    if (part_num >= MAX_PARTITIONS) {
        NGLOG_ERROR("Dv:partAdd: ignoring part_num = %d (max: %d)\n", part_num, MAX_PARTITIONS-1);
    } else {
...
#ifdef PARTITION_DEBUG
        NGLOG_DEBUG("Dv:partAdd: Got all partitions - ready to rock!");
#endif
...
#ifdef PARTITION_DEBUG
        NGLOG_DEBUG("Dv:partAdd: pending %d disk", mPendingPartCount);
#endif
    }
}
...
void DirectVolume::handleDiskChanged(const char * /*devpath*/,
    ...
    NGLOG_INFO("Volume %s disk has changed", getLabel());
    const char *tmp = evt->findParam("NPARTS");
    if (tmp) {
        mDiskNumParts = atoi(tmp);
    } else {
...
void DirectVolume::handlePartitionChanged(const char * /*devpath*/,
    ...
    int major = atoi(evt->findParam("MAJOR"));
    int minor = atoi(evt->findParam("MINOR"));
    NGLOG_DEBUG("Volume %s %s partition %d:%d changed\n", getLabel(), getMountpoint(), major, minor);
}
...
void DirectVolume::handleDiskRemoved(const char * /*devpath*/,
    ...
    NGLOG_DEBUG("Volume %s %s disk %d:%d removed\n", getLabel(), getMountpoint(), major, minor);
    snprintf(msg, sizeof(msg), "Volume %s %s disk removed (%d:%d)", getLabel(), getFuseMountpoint(), major, minor);
...
void DirectVolume::handlePartitionRemoved(const char * /*devpath*/,
    ...
    NGLOG_DEBUG("Volume %s %s partition %d:%d removed\n", getLabel(), getMountpoint(), major, minor);
    ...
    if ((dev_t) MKDEV(major, minor) == mCurrentlyMountedKdev) {
        ...
        /*if (providesAsec && mVm->cleanupAsec(this, true)) {
            NGLOG_ERROR("Failed to cleanup ASEC - unmount will probably fail!");
        }*/

        snprintf(msg, sizeof(msg), "Volume %s %s bad removal (%d:%d)", getLabel(), getFuseMountpoint(), major, minor);
        //mVm->getBroadcaster()->sendBroadcast(ResponseCode::VolumeBadRemoval, msg, false);

        if (Volume::unmountVol(true, false)) {
            NGLOG_ERROR("Failed to unmount volume on bad removal (%s)", strerror(errno));
            // XXX: At this point we're screwed for now
        } else {
            NGLOG_DEBUG("Crisis averted");
        }
    } else if (state == Volume::State_Shared) {
        /* removed during mass storage */
        snprintf(msg, sizeof(msg), "Volume %s bad removal (%d:%d)",  getLabel(), major, minor);
        /*mVm->getBroadcaster()->sendBroadcast(ResponseCode::VolumeBadRemoval, msg, false);

        if (mVm->unshareVolume(getLabel(), "ums")) {
            NGLOG_ERROR("Failed to unshare volume on bad removal (%s)", strerror(errno));
        } else {
            NGLOG_DEBUG("Crisis averted");
        }*/
    }
...
int DirectVolume::updateDeviceInfo(char *new_path, int new_major, int new_minor)
{
    ...
    if (mPartIdx == -1) {
        NGLOG_ERROR("Can only change device info on a partition\n");
        return -1;
    }
     ...
     */
    if (mPaths->size() != 1) {
        NGLOG_ERROR("Cannot change path if there are more than one for a volume\n");
        return -1;
    }
