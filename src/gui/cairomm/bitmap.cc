...
UINT Bitmap::GetPixel( int i, int j ) const {
    ...
    NGLOG_WARN("EasyBMP Warning: Attempted to access non-existent pixel"
...
bool Bitmap::SetColor( int ColorNumber , RGBAColor NewColor ) {
    if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 ) {
        NGLOG_WARN("EasyBMP Warning: Attempted to change color table for a BMP object"
                   "that lacks a color table. Ignoring request.");
        return false;
    }
    if( !Colors ) {
        NGLOG_WARN("EasyBMP Warning: Attempted to set a color, but the color table"
                   "  is not defined. Ignoring request.");
        return false;
    }
    if( ColorNumber >= TellNumberOfColors() ) {
        NGLOG_WARN("EasyBMP Warning: Requested color number %d is outside the allowed range"
...
RGBAColor Bitmap::GetColor( int ColorNumber ) {
    ...
    if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 ) {
        NGLOG_WARN("EasyBMP Warning: Attempted to access color table for a BMP object"
                   " that lacks a color table. Ignoring request.");
        return Output;
    }
    if( !Colors ) {
        NGLOG_WARN("EasyBMP Warning: Requested a color, but the color table"
                   " is not defined. Ignoring request.");
        return Output;
    }
    if( ColorNumber >= TellNumberOfColors() ) {
        NGLOG_WARN("EasyBMP Warning: Requested color number %d is outside the allowed range[0,%d]"
...
bool Bitmap::SetBitDepth( int NewDepth ) {
    ...
    if( NewDepth != 1 && NewDepth != 4 &&
        ...
        NGLOG_WARN("EasyBMP Warning: User attempted to set unsupported bit depth %d Bit depth remains unchanged at %d"
                   ,NewDepth,BitDepth);
        return false;
    }
...
bool Bitmap::SetSize(int NewWidth , int NewHeight ) {
    ...
    if( NewWidth <= 0 || NewHeight <= 0 ) {
        NGLOG_WARN("EasyBMP Warning: User attempted to set a non-positive width or height."
...
bool Bitmap::WriteToStream(std::ostream&os) {
    using namespace std;
    if(!os.good()) {
        NGLOG_INFO("EasyBMP Error: Cannot open file stream for output.");
        return false;
    }
...
// write the pixels
    ...
    if( BitDepth != 16 ) {
        ...
        while( j > -1 ) {
            ...
            if( !Success ) {
                NGLOG_INFO("EasyBMP Error: Could not write proper amount of data.");
                j = -1;
            }
            j--;
        }
...
bool Bitmap::ReadFromStream(std::istream&is) {
    using namespace std;
    if( !is.good() ) {
        NGLOG_WARN("EasyBMP Error: Cannot open stream for input.");
        SetBitDepth(1);
        SetSize(1,1);
        return false;
    }
...
// read the file header
    ...
    if( !IsBitmap ) {
        NGLOG_WARN("EasyBMP Error:stream is not a Windows BMP file!");
        return false;
    }
...
// future idea: check to see if at least most is self-consistent
    ...
    if( !NotCorrupted ) {
        NGLOG_WARN("EasyBMP Error: is obviously corrupted.");
        SetSize(1,1);
        SetBitDepth(1);
        return false;
    }
...
// if bmih.biCompression 1 or 2, then the file is RLE compressed
    ...
    if( bmih.biCompression == 1 || bmih.biCompression == 2 ) {
        NGLOG_WARN("EasyBMP Error: stream is (RLE) compressed."
...
// it's probably an OS2 bitmap file.
    ...
    if( bmih.biCompression > 3 ) {
        NGLOG_WARN("EasyBMP Error: stream is in an unsupported format. (bmih.biCompression = %d)"
    ...
    if( bmih.biCompression == 3 && bmih.biBitCount != 16 ) {
        NGLOG_WARN("EasyBMP Error: stream uses bit fields and is not a 16-bit file. This is not supported.");
        SetSize(1,1);
        SetBitDepth(1);
        return false;
    }
...
// set the bit depth
    ...
    if(    TempBitDepth != 1  && TempBitDepth != 4
        ...
        NGLOG_WARN("EasyBMP Error: unrecognized bit depth.");
        SetSize(1,1);
        SetBitDepth(1);
        return false;
    }
...
// set the size
    ...
    if( (int) bmih.biWidth <= 0 || (int) bmih.biHeight <= 0 ) {
        NGLOG_WARN("EasyBMP Error: non-positive width or height.");
        SetSize(1,1);
        SetBitDepth(1);
        return false;
    }
...
// if < 16 bits, read the palette
    ...
    if( BitDepth < 16 ) {
        ...
        NGLOG_WARN_IF(NumberOfColorsToRead < TellNumberOfColors(),
...
// skip blank data if bfOffBits so indicates
    ...
    if( BytesToSkip > 0 && BitDepth != 16 ) {
        NGLOG_WARN("EasyBMP Warning: Extra meta data detected  Data will be skipped.");
        BYTE* TempSkipBYTE;
...
// with a more-efficient buffered technique.
    ...
    if( BitDepth != 16 ) {
        ...
        while( j > -1 ) {
            ...
            if( BytesRead < BufferSize ) {
                j = -1;
                NGLOG_WARN("EasyBMP Error: Could not read proper amount of data.");
            } else {
                bool Success = false;
                if( BitDepth == 1  ) {
                ...
                if( !Success ) {
                    NGLOG_WARN("EasyBMP Error: Could not read enough pixel data!");
                    j = -1;
                }
            }
            j--;
    ...
    if( BitDepth == 16 ) {
        ...
        if( BytesToSkip > 0 ) {
            NGLOG_WARN("EasyBMP Warning: Extra meta data detected in file Data will be skipped.");
            BYTE* TempSkipBYTE;
            TempSkipBYTE = new BYTE [BytesToSkip];
            SafeRead( (char*) TempSkipBYTE , BytesToSkip , 1 ,is);
            delete [] TempSkipBYTE;
...
bool Bitmap::CreateStandardColorTable( void ) {
    using namespace std;
    if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 ) {
        NGLOG_WARN("EasyBMP Warning: Attempted to create color table at a bit"
